...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'verfiy_apps') {
        ...
        if (!empty($_POST['windows_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.freunde.io/access_token.php?code={$windows_code}&type=windows_desktop", false, stream_context_create($arrContextOptions));
        ...
        if (!empty($_POST['ios_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.freunde.io/access_token.php?code={$windows_code}&type=ios", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
